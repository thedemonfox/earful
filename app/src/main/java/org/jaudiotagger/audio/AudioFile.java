...
public class AudioFile
{
    ...
    */
    ...
    {
        if(Constants.DEBUG) logger.info("Reading file:" + "path" + file.getPath() + ":abs:" + file.getAbsolutePath());
        if (!file.exists())
        {
            if(Constants.WARN) logger.severe("Unable to find:" + file.getPath());
     ...
     * @return
     */
    protected RandomAccessFile checkFilePermissions(File file) throws FileNotFoundException
    {
        RandomAccessFile newFile;

        checkFileExists(file);

        // Unless opened as read-only the file must be writable

...
public class MP3File extends AudioFile
{
  //  private static final int MINIMUM_FILESIZE = 150;
   ...
   // private AbstractLyrics3 lyrics3tag = null;
     ...
     */
    public MP3File(String filename) throws IOException, TagException, InvalidAudioFrameException
    {
    ...
    private void readV1Tag(File file, RandomAccessFile newFile, int loadOptions) throws IOException
    {
        if ((loadOptions & LOAD_IDV1TAG) != 0)
        {
            ...
            {
                if(Constants.DEBUG) logger.info("No ids3v11 tag found");
            }
            ...
            {
                if(Constants.DEBUG) logger.info("No id3v1 tag found");
            }
        }
    }
    ...
    private void readV2Tag(File file, int loadOptions) throws IOException, TagException
    {
        ...
        if (startByte >= AbstractID3v2Tag.TAG_HEADER_LENGTH)
        {
            ...
            {
                if(Constants.DEBUG) logger.info("Attempting to read id3v2tags");
                try
                {
                ...
                {
                    if(Constants.DEBUG) logger.info("No id3v24 tag found");
                }
                ...
                {
                    if(Constants.DEBUG) logger.info("No id3v23 tag found");
                }
                ...
                {
                    if(Constants.DEBUG) logger.info("No id3v22 tag found");
                }
            }
        }
        else
        {
            if(Constants.DEBUG) logger.info("Not enough room for valid id3v2 tag:" + startByte);
        }
    }
    ...
    private MP3AudioHeader checkAudioStart(long startByte, MP3AudioHeader currentHeader) throws IOException, InvalidAudioFrameException
    {
        ...
        //because we can't agree on start location we reread the audioheader from the start of the file, at least
        //this way we can't overwrite the audio although we might overwrite part of the tag if we write this file
        //back later
        newAudioHeader = new MP3AudioHeader(file, 0);
        if(Constants.DEBUG) logger.info("Checking from start:" + newAudioHeader);
        ...
        if (currentHeader.getMp3StartByte() == newAudioHeader.getMp3StartByte())
        {
            ...
            //problem
            if(Constants.DEBUG) logger.info(ErrorMessage.MP3_START_OF_AUDIO_CONFIRMED.getMsg(file.getPath(), Hex.asHex(newAudioHeader.getMp3StartByte())));
            return currentHeader;
        }
        else
        {
            //We get a different value if read from start, can't guarantee 100% correct lets do some more checks
            if(Constants.DEBUG) logger.info((ErrorMessage.MP3_RECALCULATED_POSSIBLE_START_OF_MP3_AUDIO.getMsg(file.getPath(), Hex.asHex(newAudioHeader.getMp3StartByte()))));
            ...
            if (currentHeader.getNumberOfFrames() != newAudioHeader.getNumberOfFrames())
            {
                ...
                if(Constants.DEBUG) logger.info("Checking next:" + nextAudioHeader);
                ...
                {
                    if(Constants.WARN) logger.warning((ErrorMessage.MP3_START_OF_AUDIO_CONFIRMED.getMsg(file.getPath(), Hex.asHex(currentHeader.getMp3StartByte()))));
                    return currentHeader;
                }
                ...
                {
                    if(Constants.WARN) logger.warning((ErrorMessage.MP3_RECALCULATED_START_OF_MP3_AUDIO.getMsg(file.getPath(), Hex.asHex(newAudioHeader.getMp3StartByte()))));
                    return newAudioHeader;
                }
                ///Not sure but safer to return earlier audio because stops jaudiotagger overwriting when writing tag
                else
                {
                    if(Constants.WARN) logger.warning((ErrorMessage.MP3_RECALCULATED_START_OF_MP3_AUDIO.getMsg(file.getPath(), Hex.asHex(newAudioHeader.getMp3StartByte()))));
                    return newAudioHeader;
     ...
     * @throws org.jaudiotagger.audio.exceptions.InvalidAudioFrameException
     */
    public MP3File(File file, int loadOptions) throws IOException, TagException, InvalidAudioFrameException
    {
        ...
        try
        {
            ...
            {
                if(Constants.DEBUG) logger.info("First header found after tag:" + audioHeader);
    ...
    */
     ...
     * @throws org.jaudiotagger.audio.exceptions.InvalidAudioFrameException
     */
    public MP3File(File file) throws IOException, TagException, InvalidAudioFrameException
    {
        this(file, LOAD_ALL);
    }
    ...
    {
        if(Constants.DEBUG) logger.info("setting tagv1:v1 tag");
    ...
    {
        if(Constants.DEBUG) logger.info("setting tagv1:abstract");

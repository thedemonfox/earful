...
abstract public class AbstractID3v1Tag extends AbstractID3Tag
{
    ...
    public void delete(RandomAccessFile file) throws IOException
    {
        ...
        if(Constants.DEBUG) logger.info("Deleting ID3v1 from file if exists");
        ...
        {
            if(Constants.DEBUG) logger.config("Deleted ID3v1 tag");
            file.setLength(file.length() - TAG_LENGTH);
        }
        else
        {
            if(Constants.DEBUG) logger.config("Unable to find ID3v1 tag to deleteField");
        }
    }

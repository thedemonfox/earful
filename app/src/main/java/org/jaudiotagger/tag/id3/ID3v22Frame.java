...
public class ID3v22Frame extends AbstractID3v2Frame
{
    ...
    public ID3v22Frame(String identifier)
    {
        ...
        if(Constants.DEBUG) logger.info("Creating empty frame of type" + identifier);
        ...
        if(Constants.DEBUG) logger.info("Created empty frame of type" + this.identifier + "with frame body of" + bodyIdentifier);
    ...
    {
        super(frame);
        if(Constants.DEBUG) logger.info("Creating frame from a frame of same version");
    }
    ...
    private void createV22FrameFromV23Frame(ID3v23Frame frame) throws InvalidFrameException
    {
        ...
        if (identifier != null)
        {
            if(Constants.DEBUG) logger.info("V2:Orig id is:" + frame.getIdentifier() + ":New id is:" + identifier);
        ...
        else if (ID3Tags.isID3v23FrameIdentifier(frame.getIdentifier()))
        {
            ...
            if (identifier != null)
            {
                if(Constants.DEBUG) logger.info("V2:Force:Orig id is:" + frame.getIdentifier() + ":New id is:" + identifier);
        ...
        else if (frame.getBody() instanceof FrameBodyDeprecated)
        {
            ...
            if (ID3Tags.isID3v22FrameIdentifier(frame.getIdentifier()))
            {
                ...
                if(Constants.DEBUG) logger.info("DEPRECATED:Orig id is:" + frame.getIdentifier() + ":New id is:" + identifier);
            }
        ...
        else
        {
            ...
            identifier = frame.getIdentifier();
            if(Constants.DEBUG) logger.info("v2:UNKNOWN:Orig id is:" + frame.getIdentifier() + ":New id is:" + identifier);
        }
    }
    ...
    {
        if(Constants.DEBUG) logger.info("Creating frame from a frame of a different version");
        ...
        if(Constants.DEBUG) logger.info("Created frame from a frame of a different version");
    }
    ...
    public void read(ByteBuffer byteBuffer) throws InvalidFrameException, InvalidDataTypeException
    {
        ...
        {
            if(Constants.DEBUG) logger.info("Invalid identifier:" + identifier);
    ...
    {
        if(Constants.DEBUG) logger.info("Write Frame to Buffer" + getIdentifier());

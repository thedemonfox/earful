...
public abstract class AbstractID3v2FrameBody extends AbstractTagFrameBody
{
    ...
    public void read(ByteBuffer byteBuffer) throws InvalidTagException
    {
        ...
        if(Constants.DEBUG) logger.info("Reading body for" + this.getIdentifier() + ":" + size);
    ...
    {
        if(Constants.DEBUG) logger.info("Writing frame body for" + this.getIdentifier() + ":Est Size:" + size);
        ...
        setSize();
        if(Constants.DEBUG) logger.info("Written frame body for" + this.getIdentifier() + ":Real Size:" + size);

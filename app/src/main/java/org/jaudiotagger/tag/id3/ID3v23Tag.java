...
import org.jaudiotagger.Constants;
...
public class ID3v23Tag extends AbstractID3v2Tag
{
    ...
    {
        if(Constants.DEBUG) logger.info("Copying primitives");
    ...
    protected void translateFrame(AbstractID3v2Frame frame)
    {
        ...
        if (!tmpBody.getYear().equals(""))
        {
            ...
            if(Constants.DEBUG) logger.info("Adding Frame:" + newFrame.getIdentifier());
    ...
    public ID3v23Tag(ID3v23Tag copyObject)
    {
        ...
        super(copyObject);
        if(Constants.DEBUG) logger.info("Creating tag from another tag of same type");
    ...
    {
        if(Constants.DEBUG) logger.info("Creating tag from a tag of a different version");
        frameMap = new LinkedHashMap<String, Object>();
        encryptedFrameMap = new LinkedHashMap<String, Object>();
        ...
        if (mp3tag != null)
        {
            ...
            if(Constants.DEBUG) logger.info("Created tag from a tag of a different version");
        }
    }
    ...
    private void readHeaderFlags(ByteBuffer buffer) throws TagException
    {
        ...
        {
            if(Constants.DEBUG) logger.info(ErrorMessage.ID3_TAG_UNSYNCHRONIZED.getMsg(getLoggingFilename()));
        }

        if (extended)
        {
            if(Constants.DEBUG) logger.info(ErrorMessage.ID3_TAG_EXTENDED.getMsg(getLoggingFilename()));
        }

        if (experimental)
        {
            if(Constants.DEBUG) logger.info(ErrorMessage.ID3_TAG_EXPERIMENTAL.getMsg(getLoggingFilename()));
        }
    }
    ...
    private void readExtendedHeader(ByteBuffer buffer, int size)
    {
        ...
        if (extendedHeaderSize == TAG_EXT_HEADER_DATA_LENGTH)
        {
            ...
            if(paddingSize>0)
            {
                if(Constants.DEBUG) logger.info(ErrorMessage.ID3_TAG_PADDING_SIZE.getMsg(getLoggingFilename(),paddingSize));
            }
        ...
        {
            if(Constants.DEBUG) logger.info(ErrorMessage.ID3_TAG_CRC.getMsg(getLoggingFilename()));
            ...
            //CRC Data
            crc32 = buffer.getInt();
            if(Constants.DEBUG) logger.info(ErrorMessage.ID3_TAG_CRC_SIZE.getMsg(getLoggingFilename(),crc32));
        }
    ...
    public void read(ByteBuffer buffer) throws TagException
    {
        ...
        if(Constants.DEBUG) logger.info(getLoggingFilename() + ":" + "Reading ID3v23 tag");

        readHeaderFlags(buffer);

        // Read the size, this is size of tag not including the tag header
        size = ID3SyncSafeInteger.bufferToValue(buffer);
        if(Constants.DEBUG) logger.info(ErrorMessage.ID_TAG_SIZE.getMsg(getLoggingFilename(),size));
        ...
        if(Constants.DEBUG) logger.info(getLoggingFilename() + ":Loaded Frames,there are:" + frameMap.keySet().size());
